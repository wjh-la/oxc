    ...
    pub fn build(&self, root_uri: &Uri, options: serde_json::Value) -> ServerLinter {
        ...
        let oxlintrc = if let Some(config_path) = config_path {
            ...
            if config.try_exists().is_ok_and(|exists| exists) {
                if config.ends_with(DEFAULT_OXLINT_CONFIG_TS) {
                    #[cfg(feature = "napi")]
                    let config = Self::load_ts_config(&config, self.js_config_loader.as_ref());
                    #[cfg(not(feature = "napi"))]
                    let config = {
                        warn!(
                            "TypeScript config files ({DEFAULT_OXLINT_CONFIG_TS}) are not supported in this build."
                        );
                        None
                    };
            ...
            let ts_path = root_path.join(DEFAULT_OXLINT_CONFIG_TS);
...
impl ServerLinterBuilder {
    ...
    fn create_nested_configs(
        ...
        for path in paths {
            ...
            } else if file_path.file_name().is_some_and(|name| name == DEFAULT_OXLINT_CONFIG_TS) {
        ...
        if !js_config_paths.is_empty() {
            if let Some(loader) = js_config_loader {
                ...
                warn!(
                    "TypeScript config files ({DEFAULT_OXLINT_CONFIG_TS}) found but JS runtime not available."
                );
            }
        }
...
impl Tool for ServerLinter {
    ...
    fn get_watcher_patterns(&self, options: serde_json::Value) -> Vec<Pattern> {
        ...
        let mut watchers = match options.config_path.as_deref() {
            Some("") | None => {
                vec![format!("**/{DEFAULT_OXLINTRC}"), format!("**/{DEFAULT_OXLINT_CONFIG_TS}")]
            }
...
mod test {
    ...
    fn test_ts_config_root_snapshot() {
        ...
        let expected_path = root.join(crate::DEFAULT_OXLINT_CONFIG_TS);
